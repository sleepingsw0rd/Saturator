...
- Follows C++17 standards and JUCE framework conventions
...
- **ALWAYS use test-runner agent** to run builds and tests - never execute cmake directly with Bash
- Test results must be obtained from test-runner agent which provides detailed error reporting
- Code compiles without warnings on MSVC (C++17)
- Adequate verification for new functionality
- Performance optimized without sacrificing readability (critical for real-time audio)
- No obvious bottlenecks or inefficiencies in the audio processing path

**Test Integrity Violations (CRITICAL)**

- **ABSOLUTELY NO TEST SHORTCUTS**: Flag any attempts to bypass failing tests
- Detect commented out assertions or verification code
- Identify disabled or bypassed build checks
...
- Identify code that has been modified to match incorrect implementation rather than fixing the implementation
- Flag any logic that has been artificially simplified to compile
- **If build issues are genuinely difficult to fix**: Stop and ask the user for guidance with a detailed explanation of why they are challenging
...
```cpp
// ❌ CRITICAL: Assertion commented out to avoid failure
void testSaturation()
{
    // jassert(saturate(1.5f, SaturationMode::HardClip) <= 1.0f);
}

// ❌ CRITICAL: Build warning suppressed instead of fixing implementation
#pragma warning(disable: 4244) // hiding real conversion issue

// ❌ CRITICAL: Irrelevant code added just to compile
float processBlock(float sample)
{
    return 0.0f; // placeholder instead of actual implementation
}
```
...
- **Example**:
  ...
  ```cpp
  // ⚠️ WARNING: Missing null check on parameter value
  float drive = apvts.getRawParameterValue("drive")->load();

  // ✅ FIX: Add null safety
  auto* driveParam = apvts.getRawParameterValue("drive");
  float drive = driveParam != nullptr ? driveParam->load() : 1.0f;
  ```
...
- **Example**:
  ...
  static constexpr float kMaxSoftClipOutput = 2.0f / 3.0f;
  if (driven > 1.0f)
      return kMaxSoftClipOutput;
  ```
